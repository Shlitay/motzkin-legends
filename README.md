...
- **Jackpot badge** (`JackpotBadge.tsx`) — gold gradient pill, approved-participant count × a hardcoded ₪20 entry fee (`ENTRY_FEE_ILS` in the component — not manager-configurable, revisit if the buy-in ever changes). Hides itself if there's no open round.
