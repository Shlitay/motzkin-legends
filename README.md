...
- **0-0 baseline at kickoff**: `lock_expired_rounds()` now seeds every still-unscored match to `0-0` (`is_final = false`) the instant a round locks, and computes initial `predictions.points_earned`/standings from it — "empty" isn't a real state once the round has begun. Only touches matches with `home_score is null`, so a result the manager may have already entered pre-lock is never clobbered.
