...
15. `update-lock-expired-rounds-seed-scores.sql` — extends `lock_expired_rounds()` (migration 8) to seed every still-unscored match to 0-0 the moment a round locks, and compute initial points/standings from that baseline. `create or replace`, same signature, safe over the existing function. **Not yet applied.**
16. `update-submit-match-result-final-flag.sql` — extends `submit_match_result()` with a 4th parameter, `p_is_final`; the round's own `status` now follows its matches (`'finished'` once every match in the round is final, reverts to `'locked'` if a final gets unchecked). Drops the old 3-argument function first, since the signature changed. **Not yet applied — the manager's "המשחק הסתיים" checkbox won't save until this runs.**
