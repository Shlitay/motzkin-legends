...
13. `add-match-is-final.sql` — adds `matches.is_final` (default `false`), tracking whether a score is live or final. **Not yet applied.**
14. `add-recompute-round-standings-function.sql` — extracts the aggregate/rank recompute out of `submit_match_result()` into a shared `recompute_round_standings(round_id)`, so `lock_expired_rounds()` can call the same logic. **Not yet applied — must run before migrations 15 and 16.**
